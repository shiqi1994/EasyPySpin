...
class VideoCapture:
    ...
    def __init__(self, index):
        ...
        if num_cam==0:
            print >> stderr, "EasyPySpin: no camera is available"
        ...
        try:
            ...
            if type(index) is int:
                ...
                if index<0 or num_cam-1<index:
                    # print "EasyPySpin: out device of bound"
                    # print >> stderr, f"EasyPySpin: out device of bound (0-{num_cam-1}): {index}"
                    print >> stderr, "EasyPySpin: out device of bound"
                self.cam = self._cam_list.GetByIndex(index)
            # Serial case
            elif type(index) is str:
                self.cam = self._cam_list.GetBySerial(index)
        except:
            print >> stderr, "EasyPySpin: camera failed to properly initialize!"
            self._cam_list.Clear()
            self._system.ReleaseInstance()
            return None
